...
- **Community hall card:** Marc has no photos of the renovation and asked for no placeholder. The card uses two more facts from his resume instead (escalating issues, keeping the client and stakeholders updated). At his request it also has a generic background so it matches the photo card: two public-domain (CC0 1.0) photos of a building renovation by Lav Ulv on Flickr (flickr.com/photos/99460566@N06/36017586220 and /35578878314), re-encoded without metadata. Marc asked for no captions on the backgrounds, so the stock photos are unlabelled decoration; the card text makes no claim about them.
- **Coursework card:** at Marc's request its background shows the four main topics, one public-domain (CC0 1.0) photo each, cycling in the same order as the list: road geometry (cloverleaf interchange, rawpixel.com/image/9658247), earthworks (Oosterdokseiland, Amsterdam, by Fons Heijnsbroek, Wikimedia Commons curid 26482140), pavement (resurfacing, rawpixel.com/image/9677541) and stormwater drainage (pipe laying, rawpixel.com/image/9677505). All re-encoded as JPEG without metadata. All three project cards use the same see-through sheet, fade, pause and contrast rules; the lowest measured text contrast across every background is 4.78:1.
