...
- **Road cross-section redraw:** at Marc's request the cross-section was redrawn as an SVG with presentation fixes only: title block, NTS, centreline, crossfall (2.5% lanes, 4% shoulders, from his report), millimetre dimensions and corrected notes. His design is unchanged except that the service-conduit graphic (drawn under the whole road, real offset unknown) was removed, and layer depths are left to the pavement design. The caption and the drawing's revision table both say it was redrawn.
