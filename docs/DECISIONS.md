...
- **Community hall card:** Marc has no photos of the renovation and asked for no placeholder. The card uses two more facts from his resume instead (escalating issues, keeping the client and stakeholders updated). At his request it also has a generic background so it matches the photo card: two public-domain (CC0 1.0) photos of a building renovation by Lav Ulv on Flickr (flickr.com/photos/99460566@N06/36017586220 and /35578878314), re-encoded without metadata. The card says they are stock photos for illustration and not from this project, so no one reads them as his job.
- **Coursework card:** its background is his own Revit bridge and AutoCAD footing models (the same files as the Drawings section), labelled as such. All three project cards use the same see-through sheet, cross-fade, pause and contrast rules; the lowest measured text contrast across all three is 4.78:1.
